...
async def submit_document(
    ...
    try:
        ...
        course = get_course_by_id_or_code(
            connection,
            clean_course_input,
        )
        ...
        if not course:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=(
                    f"Course '{clean_course_input}' does not exist. "
        ...
        if not is_student_enrolled(
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=(
                    f"You are not enrolled in "
                    f"'{course['code']} - {course['name']}'. "
                    "Please enroll in the course before submitting."
                ),
            )
        ...
        unique_file_name = (
            f"{uuid.uuid4().hex}_{safe_orig_name}"
        )
        ...
        # Compare against reference corpus and prior peer submissions
        ...
        analysis_result = calculate_similarity(
            processed_text,
            historical_documents=historical_docs,
        )

        overall_similarity = analysis_result[
        ...
        # Store explainable similarity evidence
        evidence_details = {
            "similarity_evidence": analysis_result.get(
                "evidence_summary",
                {},
            )
        }
...
def download_submission_file(
    ...
    try:
        ...
        if user_role == "student":
            ...
            if (
                not course_row
                or course_row["professor_id"] != user_id_int
            ):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=(
                        "You are not authorized to download "
                        "submissions for courses you do not teach."
                    ),
                )
        ...
        download_filename = (
            submission["filename"] or safe_name
        )
